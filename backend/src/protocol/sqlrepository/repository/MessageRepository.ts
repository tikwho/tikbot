...
export class SqliteMessageRepository extends Repository<MessageEntity> {
     ...
     * 
     */
     async getPageList( team_id:string, pageNum:number,pageSize?:number,from_id?:string,conversation_id?:string,last_datetime?:any,keyword?:string,types?:string[]|string) {
        ...
        if(keyword && keyword.length>0) queryBuilder = await queryBuilder.andWhere(`( message.content like :keyword or message.content_trans like :keyword )`, {'keyword':'%'+keyword+'%'});
        if(types) queryBuilder = await queryBuilder.andWhere(`message.type in (:types)`, {'types':"string"==typeof types?[types]:types});
    ...
    async createOrUpdate(fillData:any,message_id?:any) {
        ...
        let entityObj = "string"==typeof message_id ? await MessageEntity.findOne( message_id ) : message_id;//{where:{id:message_id}}
        // if( fillData&&fillData["uid"] ){ entityObj =  await MessageEntity.findOne({where:{uid:fillData["uid"]} });}
        if(!entityObj){ entityObj = await new MessageEntity();}
