...
function write(e) {
    var a = "string" == typeof e ? function (e) {
        let n = encodeURIComponent(e)['replace'](/%([0-9A-F]{2})/g, (function (e, r) {
...
function sign(url_search_params, user_agent, args=[0, 1, 8]) {
    /**
...
// }
// https://github.com/ShilongLee/Crawler/blob/e3490ea66a93778ffd91d1bc002dc69bdf4df313/service/douyin/logic/common.py#L131
// https://github.com/zhouxuanbo1990/get_a_bogus/blob/main/a_bogus.js#L11172
...
//   }
  ...
  sign(url, params?:any, userAgent1?:string,data?:any,type?:string) {
    // if (userAgent) {
    ...
    if(params){
        for (let k in params) {
            uri.searchParams.set(k, params[k]);
        }
    }

    if(type=="im"){
        // console.info(uri.searchParams.toString(),userAgent1||userAgent,data);
        // return console.info(get_x_bogus(uri.searchParams.toString(),userAgent1||userAgent,data));
        uri.searchParams.set("a_bogus", sign(decodeURIComponent(uri.searchParams.toString()),userAgent1||userAgent,data));
    }
    else{
        // uri.searchParams.set("msToken", this.msToken()) ;
        console.info(uri.searchParams.toString(),userAgent,data||[0, 1, 14]);
        uri.searchParams.set("a_bogus", sign(uri.searchParams.toString(),userAgent,data||[0, 1, 14]));
        // uri.searchParams.append("X-Bogus", get_x_bogus(decodeURIComponent(uri.searchParams.toString()),userAgent,data));
        // uri.searchParams.set("_signature", this.signature(url)) ;
    }
