...
import TikTokHttpClient from './lib/tiktokHttpClient';
import WebcastWebsocket from './lib/webcastWebsocket';
import { getRoomIdFromMainPageHtml, validateAndNormalizeUniqueId } from './lib/tiktokUtils';
import { simplifyObject } from './lib/webcastDataConverter';
import { deserializeMessage, deserializeWebsocketMessage } from './lib/webcastProtobuf';

import Config from './lib/webcastConfig';
...
export class WebcastPushConnection extends EventEmitter {
    ...
    async getRoomInfo() {
        const _this = this;
        if(this.fetchRooming){
            let promise3 = new Promise((resolve, reject) => {
                const checkInterval = setInterval(function(){ 
                    if(!_this.fetchRooming){
                        clearInterval(checkInterval);
                        resolve( _this.fetchRooming);
                    }
                },500);
            });
            return await await Promise.resolve(promise3);
        }//请求锁

        this.fetchRooming = true;
        try {
    ...
    async retrieveRoomId() {
        for(let i=0;i<3;i++){
            try {
                // console.info(`@${this.uniqueStreamerId}/live`,"mainPageHtml-----");
                let mainPageHtml = await this.httpClient.getMainPage(`@${this.uniqueStreamerId}/live`);
                try {
                    // console.info(`@${this.uniqueStreamerId}/live`,mainPageHtml,"mainPageHtml-----");
                    ...
                    this.roomId = this.roomId||roomData.data.user.roomId;
                    this.clientParams.room_id = this.roomId;
                }
    ...
    async enterRoomInfo() {
        try {
            let response = await this.httpClient.postFormDataToWebcastApi('room/enter/', this.clientParams,{enter_source:"live_merge-live_cover",room_id:this.roomId});
            return response.data;
        } catch (err) {
            throw new Error(`Failed to enter room ${err.message}`);
        }
    }
    ...
    async fetchRoomData(isInitial) {
        ...
        let upgradeToWsOffered = !!webcastResponse.wsUrl && !!webcastResponse.wsParams;
        // if(webcastResponse.wsUrl){
        //     console.info(webcastResponse,"webcast im/fetch Response------------------- -");
        // }
        ...
        console.info(this.isWsUpgradeDone,this.options.enableWebsocketUpgrade && upgradeToWsOffered,"ws-----------------------");
        if (!this.isWsUpgradeDone) {
    ...
    async tryUpgradeToWebsocket(webcastResponse) {
        try {
            ...
            webcastResponse.wsParams.forEach(item=>{
                wsParams[item.name] = item.value;
            });
            ...
            this.isPollingEnabled = false;//停止http请求
    ...
    async setupWebsocket(wsUrl, wsParams) {
        return new Promise((resolve, reject) => {
            ...
            this.websocket.on('webcastResponse', (msg) => this.processWebcastResponse(msg,true));
    ...
    processWebcastResponse(webcastResponse,isws?:boolean) {
        console.info(isws? "来源ws:"+this.uniqueStreamerId :"来源http :");
        ...
        // Process and emitx decoded data depending on the the message type
                ...
                switch (message.type) {
                    case 'WebcastControlMessage':
                        ...
                        if ([3, 4].includes(action)) {
                            this.emitx(ControlEvents.STREAMEND, { action });
                            this.disconnect();
                        }
                        break;
                    case 'WebcastRoomUserSeqMessage':
                        this.emitx(MessageEvents.ROOMUSER, simplifiedObj);
                        break;
                    case 'WebcastChatMessage':
                        this.emitx(MessageEvents.CHAT, simplifiedObj);
                        break;
                    case 'WebcastMemberMessage':
                        this.emitx(MessageEvents.MEMBER, simplifiedObj);
                        break;
                    case 'WebcastGiftMessage':
                        ...
                        this.emitx(MessageEvents.GIFT, simplifiedObj);
                        break;
                    case 'WebcastSocialMessage':
                        this.emitx(MessageEvents.SOCIAL, simplifiedObj);
                        if (simplifiedObj.displayType?.includes('follow')) {
                            this.emitx(CustomEvents.FOLLOW, simplifiedObj);
                        }
                        if (simplifiedObj.displayType?.includes('share')) {
                            this.emitx(CustomEvents.SHARE, simplifiedObj);
                        }
                        break;
                    case 'WebcastLikeMessage':
                        this.emitx(MessageEvents.LIKE, simplifiedObj);
                        break;
                    case 'WebcastQuestionNewMessage':
                        this.emitx(MessageEvents.QUESTIONNEW, simplifiedObj);
                        break;
                    case 'WebcastLinkMicBattle':
                        this.emitx(MessageEvents.LINKMICBATTLE, simplifiedObj);
                        break;
                    case 'WebcastLinkMicArmies':
                        this.emitx(MessageEvents.LINKMICARMIES, simplifiedObj);
                        break;
                    case 'WebcastLiveIntroMessage':
                        this.emitx(MessageEvents.LIVEINTRO, simplifiedObj);
                        break;
                    case 'WebcastEmoteChatMessage':
                        this.emitx(MessageEvents.EMOTE, simplifiedObj);
                        break;
                    case 'WebcastEnvelopeMessage':
                        this.emitx(MessageEvents.ENVELOPE, simplifiedObj);
                        break;
                    case 'WebcastSubNotifyMessage':
                        this.emitx(MessageEvents.SUBSCRIBE, simplifiedObj);
                        break;
                }
            });
    }

    emitx(event: string, data: any) {
